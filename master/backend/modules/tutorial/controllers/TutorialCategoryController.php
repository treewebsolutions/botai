...
use backend\modules\tutorial\models\TutorialCategoryForm;
use backend\modules\tutorial\models\TutorialCategorySearch;
...
class TutorialCategoryController extends MainController
{
	/**
	 * @inheritdoc
	 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-tutorial-categories'],
						'roles' => ['viewTutorialCategory'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
						'roles' => ['createTutorialCategory'],
					],
					[
						'allow' => true,
						'actions' => ['update'],
						'roles' => ['updateTutorialCategory'],
					],
					[
						'allow' => true,
						'actions' => ['delete'],
						'roles' => ['deleteTutorialCategory'],
						'verbs' => ['POST'],
					],
					[
						'allow' => true,
						'actions' => ['restore'],
						'roles' => ['restoreTutorialCategory'],
						'verbs' => ['POST'],
					],
				],
			],
	...
	{
		return [
			'dt-tutorial-categories' => TutorialCategorySearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == TutorialCategory::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreTutorialCategory')) {
	...
	{
		$model = new TutorialCategoryForm();
		$result = true;

		Yii::$app->eventLog->beginRecord($model);
		if ($model->load(Yii::$app->request->post()) && ($result = $model->save())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllTutorialCategories']));
	...
	{
		$model = $this->findModel($id, TutorialCategoryForm::class);
		$result = true;
	...
	/**
	 * Deletes existing TutorialCategory models.
	...
	/**
	 * Restores TutorialCategory models that are marked as deleted.
	...
	/**
	 * Finds the TutorialCategory model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|TutorialCategory|TutorialCategoryForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : TutorialCategory::class;
		...
		$query->andWhere(['deleted' => TutorialCategory::NO]);
