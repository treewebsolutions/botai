...
$this->title = Yii::t('common', 'Tutorial Categories');
$this->params['breadcrumbs'] = [
	$this->title,
];

if ($showTrash = Yii::$app->settings->get('enableSoftDelete')) {
	$showTrash = Yii::$app->request->get('deleted') == TutorialCategory::YES;
	$this->params['breadcrumbs'][] = [
		'template' => '<li class="page-links">{link}</li>',
		'label' => implode('', [
			Html::a(Yii::t('common', 'Current'), ['index'], ['class' => $showTrash ? '' : 'active']),
			Html::a(Yii::t('common', 'Trash'), ['index', 'deleted' => TutorialCategory::YES], ['class' => $showTrash ? 'active' : '']),
		]),
	];
}
...
$this->params['actions'] = [
	[
		'visible' => Yii::$app->user->can('restoreTutorialCategory') && $showTrash,
		'tag' => 'button',
		'icon' => 'fa fa-undo',
		'options' => [
			...
			'data' => [
				...
				'dt-table' => '#dt-tutorial-categories',
			],
		],
	],
	[
		'visible' => Yii::$app->user->can('deleteTutorialCategory'),
		'tag' => 'button',
		'icon' => 'fa fa-trash',
		'options' => [
...
<?= DataTable::widget([
	...
	'clientOptions' => [
		...
		'serverSide' => true,
		'ajax' => [
			'url' => Url::to(['dt-tutorial-categories']),
			'method' => 'POST',
			'data' => new JsExpression('function (data) {
				data.deleted = ' . json_encode($showTrash ? TutorialCategory::YES : null) . ';
			}'),
		],
		'order' => [
			[2, 'asc'],
		...
		'columns' => [
			...
			[
				...
				'className' => 'col-autowidth',
				'filter' => ['select', ArrayHelper::getColumn(TutorialCategory::getStatusLabels(), 'label')],
			],
		],
	],
]) ?>
