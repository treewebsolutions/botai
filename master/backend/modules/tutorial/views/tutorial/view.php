...
<?= DetailView::widget([
	...
	'attributes' => [
		...
		[
			...
			'value' => function ($model) {
                if ($model->imageUrl && is_file(Yii::getAlias("@uploads/tutorial/{$model->id}/{$model->image}"))) {
        ...
        [
            ...
            'value' => function (Tutorial $model) {
                if ($categories = $model->tutorialCategories) {
                    return implode('', array_map(function (TutorialCategory $category) {
                        return Html::a($category->translation->title, ['/tutorial-manager/tutorial-category/view', 'id' => $category->id], ['class' => 'btn btn-xs btn-default']);
                    }, $categories));
                }
                return '&mdash;';
            },
