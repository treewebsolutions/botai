...
use backend\modules\website\models\ServiceForm;
use backend\modules\website\models\ServiceSearch;
...
class ServiceController extends MainController
{
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-services'],
						'roles' => ['viewService'],
					],
					[
						'allow' => true,
	...
	{
		return [
			'dt-services' => ServiceSearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == Service::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreService')) {
	...
	public function actionCreate()
	{
		...
		if ($model->load(Yii::$app->request->post()) && ($result = $model->save())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllServices']));
	...
	{
		$model = $this->findModel($id, ServiceForm::class);
		$result = true;
	...
	/**
	 * Deletes existing Service models.
	...
	/**
	 * Restores Service models that are marked as deleted.
	...
	/**
	 * Finds the Service model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Service|ServiceForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Service::class;
		...
		$query->andWhere(['deleted' => Service::NO]);
