...
use backend\modules\website\models\FaqForm;
use backend\modules\website\models\FaqSearch;
...
class FaqController extends MainController
{
	...
	{
		if (Yii::$app->request->get('deleted') == SurveyQuestion::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreFaq')) {
	...
	public function actionCreate()
	{
		...
		if ($model->load(Yii::$app->request->post()) && ($result = $model->save())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllFaqs']));
	...
	{
		$model = $this->findModel($id, FaqForm::class);
		$result = true;
	...
	/**
	 * Deletes existing SurveyQuestion models.
	...
	/**
	 * Restores SurveyQuestion models that are marked as deleted.
	...
	/**
	 * Finds the SurveyQuestion model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|SurveyQuestion|FaqForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : SurveyQuestion::class;
		$query = $modelName::find()
			...
			->andWhere([
				'sq.id' => $id,
				's.type' => Survey::TYPE_FAQ,
				's.default' => Survey::YES,
			]);
		...
		$query->andWhere(['sq.deleted' => SurveyQuestion::NO]);
