...
use backend\modules\website\models\ArticleForm;
use backend\modules\website\models\ArticleSearch;
...
class ArticleController extends MainController
{
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-articles'],
						'roles' => ['viewArticle'],
					],
					[
						'allow' => true,
	...
	{
		return [
			'dt-articles' => ArticleSearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == Article::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreArticle')) {
	...
	public function actionCreate()
	{
		...
		if ($model->load(Yii::$app->request->post()) && ($result = $model->save())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllArticles']));
	...
	{
		$model = $this->findModel($id, ArticleForm::class);
		$result = true;
	...
	/**
	 * Deletes existing Article models.
	...
	/**
	 * Restores Article models that are marked as deleted.
	...
	/**
	 * Finds the Article model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Article|ArticleForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Article::class;
		...
		$query->andWhere(['deleted' => Article::NO]);
