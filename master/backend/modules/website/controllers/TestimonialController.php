...
use backend\modules\website\models\TestimonialForm;
use backend\modules\website\models\TestimonialSearch;
...
class TestimonialController extends MainController
{
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-testimonials'],
						'roles' => ['viewTestimonial'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
						'roles' => ['createTestimonial'],
					],
					[
						'allow' => true,
						'actions' => ['update', 'delete-file'],
						'roles' => ['updateTestimonial'],
					],
					[
						'allow' => true,
						'actions' => ['delete'],
						'roles' => ['deleteTestimonial'],
						'verbs' => ['POST'],
					],
					[
						'allow' => true,
						'actions' => ['restore'],
						'roles' => ['restoreTestimonial'],
						'verbs' => ['POST'],
					],
				],
			],
	...
	{
		return [
			'dt-testimonials' => TestimonialSearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == Testimonial::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreTestimonial')) {
	...
	public function actionCreate()
	{
		...
		if ($model->load(Yii::$app->request->post()) && ($result = $model->saveModel())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllTestimonials']));
	...
	{
		$model = $this->findModel($id, TestimonialForm::class);
		$result = true;
	...
	/**
	 * Deletes existing Testimonial models.
	...
	/**
	 * Restores Testimonial models that are marked as deleted.
	...
	/**
	 * Finds the Testimonial model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Testimonial|TestimonialForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Testimonial::class;
		...
		$query->andWhere(['deleted' => Testimonial::NO]);
