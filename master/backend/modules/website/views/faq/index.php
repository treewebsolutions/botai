...
if ($showTrash = Yii::$app->settings->get('enableSoftDelete')) {
	$showTrash = Yii::$app->request->get('deleted') == SurveyQuestion::YES;
	$this->params['breadcrumbs'][] = [
		'template' => '<li class="page-links">{link}</li>',
		'label' => implode('', [
			Html::a(Yii::t('common', 'Current'), ['index'], ['class' => $showTrash ? '' : 'active']),
			Html::a(Yii::t('common', 'Trash'), ['index', 'deleted' => SurveyQuestion::YES], ['class' => $showTrash ? 'active' : '']),
		]),
	];
}
...
$this->params['actions'] = [
	[
		'visible' => Yii::$app->user->can('restoreFaq') && $showTrash,
		'tag' => 'button',
		'icon' => 'fa fa-undo',
		'options' => [
	...
	'clientOptions' => [
		...
		'ajax' => [
			...
			'data' => new JsExpression('function (data) {
				data.deleted = ' . json_encode($showTrash ? SurveyQuestion::YES : null) . ';
			}'),
		],
		'order' => [
			[2, 'asc'],
		...
		'columns' => [
			...
			[
				...
				'filter' => ['select', SurveyQuestion::getBooleanLabels()],
			],
			[
				'data' => 'created_by',
			...
			[
				...
				'className' => 'col-autowidth',
				'filter' => ['select', ArrayHelper::getColumn(SurveyQuestion::getStatusLabels(), 'label')],
			],
		],
	],
]) ?>
