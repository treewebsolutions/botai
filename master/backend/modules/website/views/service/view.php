...
$this->title = Yii::t('common', 'View {item}', ['item' => Yii::t('common', 'Service')]);
...
$this->params['actions'] = [
	[
		'visible' => Yii::$app->user->can('viewService'),
		'tag' => 'a',
		'url' => ['index'],
		'icon' => 'fa fa-list',
		'options' => [
	...
	[
		'visible' => Yii::$app->user->can('updateService'),
		'tag' => 'a',
	...
	[
		'visible' => Yii::$app->user->can('deleteService'),
		'tag' => 'a',
		...
		'attributes' => [
			[
				...
				'value' => function (Service $model) {
					if ($model->image && is_file(Yii::getAlias("@uploads/service/{$model->id}/{$model->image}"))) {
			...
			[
				...
				'value' => function (Service $model) {
					if ($model->serviceCategories) {
						return implode(', ', array_map(function (ServiceCategory $serviceCategory) {
							return Html::a($serviceCategory->translation->title, ['/service-manager/service-category/view', 'id' => $serviceCategory->id]);
				...
				'clientOptions' => [
					...
					'ajax' => [
						...
						'data' => new JsExpression('function (data) {
							data.model = ' . json_encode(Service::class) . '; 
