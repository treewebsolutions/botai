...
if ($showTrash = Yii::$app->settings->get('enableSoftDelete')) {
	$showTrash = Yii::$app->request->get('deleted') == Service::YES;
	$this->params['breadcrumbs'][] = [
		'template' => '<li class="page-links">{link}</li>',
		'label' => implode('', [
			Html::a(Yii::t('common', 'Current'), ['index'], ['class' => $showTrash ? '' : 'active']),
			Html::a(Yii::t('common', 'Trash'), ['index', 'deleted' => Service::YES], ['class' => $showTrash ? 'active' : '']),
		]),
	];
}
...
$this->params['actions'] = [
	[
		'visible' => Yii::$app->user->can('restoreService') && $showTrash,
		'tag' => 'button',
		'icon' => 'fa fa-undo',
		'options' => [
...
<?= DataTable::widget([
	...
	'clientOptions' => [
		...
		'serverSide' => true,
		'ajax' => [
			'url' => Url::to(['dt-services']),
			'method' => 'POST',
			'data' => new JsExpression('function (data) {
				data.deleted = ' . json_encode($showTrash ? Service::YES : null) . ';
			}'),
		],
		'order' => [
			[2, 'asc'],
		...
		'columns' => [
			...
			[
				'data' => 'featured',
				'title' => Yii::t('label', 'Featured'),
				'filter' => ['select', Service::getBooleanLabels()],
			],
			[
				'data' => 'views',
				'title' => Yii::t('label', 'Views'),
				'filter' => ['text'],
			],
			[
				'data' => 'created_by',
			...
			[
				...
				'className' => 'col-autowidth',
				'filter' => ['select', ArrayHelper::getColumn(Service::getStatusLabels(), 'label')],
			],
		],
	],
]) ?>
