...
if ($showTrash = Yii::$app->settings->get('enableSoftDelete')) {
	$showTrash = Yii::$app->request->get('deleted') == Article::YES;
	$this->params['breadcrumbs'][] = [
		'template' => '<li class="page-links">{link}</li>',
		'label' => implode('', [
			Html::a(Yii::t('common', 'Current'), ['index'], ['class' => $showTrash ? '' : 'active']),
			Html::a(Yii::t('common', 'Trash'), ['index', 'deleted' => Article::YES], ['class' => $showTrash ? 'active' : '']),
		]),
	];
}
...
$this->params['actions'] = [
	[
		'visible' => Yii::$app->user->can('restoreArticle') && $showTrash,
		'tag' => 'button',
		'icon' => 'fa fa-undo',
		'options' => [
...
<?= DataTable::widget([
	...
	'clientOptions' => [
		...
		'serverSide' => true,
		'ajax' => [
			'url' => Url::to(['dt-articles']),
			'method' => 'POST',
			'data' => new JsExpression('function (data) {
				data.deleted = ' . json_encode($showTrash ? Article::YES : null) . ';
			}'),
		],
		'order' => [
			[7, 'desc'],
		...
		'columns' => [
			...
			[
				...
				'filter' => ['select', ArrayHelper::getColumn(Article::getStatusLabels(), 'label')],
			],
		],
	],
]) ?>
