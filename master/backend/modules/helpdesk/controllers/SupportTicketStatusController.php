...
use backend\modules\helpdesk\models\SupportTicketStatusForm;
use backend\modules\helpdesk\models\SupportTicketStatusSearch;
...
class SupportTicketStatusController extends MainController
{
	/**
	 * @inheritdoc
	 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-support-ticket-statuses'],
						'roles' => ['viewHelpdeskSupportTicketStatus'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
						'roles' => ['createHelpdeskSupportTicketStatus'],
					],
					[
						'allow' => true,
						'actions' => ['update'],
						'roles' => ['updateHelpdeskSupportTicketStatus'],
					],
					[
						'allow' => true,
						'actions' => ['delete'],
						'roles' => ['deleteHelpdeskSupportTicketStatus'],
						'verbs' => ['POST'],
					],
					[
						'allow' => true,
						'actions' => ['restore'],
						'roles' => ['restoreHelpdeskSupportTicketStatus'],
						'verbs' => ['POST'],
					],
				],
			],
	...
	{
		return [
			'dt-support-ticket-statuses' => SupportTicketStatusSearch::class,
		];
	}

	/**
	 * Lists all SupportTicketStatus models.
	 *
	 * @return mixed
	 */
	public function actionIndex()
	{
		if (Yii::$app->request->get('deleted') == SupportTicketStatus::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreHelpdeskSupportTicketStatus')) {
	...
	/**
	 * Displays a single SupportTicketStatus model.
	 *
	 * @param integer $id
	 * @return mixed
	...
	/**
	 * Creates a new SupportTicketStatus model.
	...
	{
		$model = new SupportTicketStatusForm();
		$result = true;

		Yii::$app->eventLog->beginRecord($model);
		if ($model->load(Yii::$app->request->post()) && ($result = $model->saveModel())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllSupportTicketStatuses']));
	...
	/**
	 * Updates an existing SupportTicketStatus model.
	...
	{
		$model = $this->findModel($id, SupportTicketStatusForm::class);
		$result = true;
	...
	/**
	 * Deletes existing SupportTicketStatus models.
	...
	/**
	 * Restores SupportTicketStatus models that are marked as deleted.
	...
	/**
	 * Finds the SupportTicketStatus model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|SupportTicketStatus|SupportTicketStatusForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : SupportTicketStatus::class;
		...
		$query->andWhere(['deleted' => SupportTicketStatus::NO]);
