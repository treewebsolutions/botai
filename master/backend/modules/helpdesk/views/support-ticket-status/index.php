...
$this->title = Yii::t('common', 'Support Ticket Statuses');
...
if ($showTrash = Yii::$app->settings->get('enableSoftDelete')) {
	$showTrash = Yii::$app->request->get('deleted') == SupportTicketStatus::YES;
	$this->params['breadcrumbs'][] = [
		'template' => '<li class="page-links">{link}</li>',
		'label' => implode('', [
			Html::a(Yii::t('common', 'Current'), ['index'], ['class' => $showTrash ? '' : 'active']),
			Html::a(Yii::t('common', 'Trash'), ['index', 'deleted' => SupportTicketStatus::YES], ['class' => $showTrash ? 'active' : '']),
		]),
	];
}
...
$this->params['actions'] = [
	[
		'visible' => Yii::$app->user->can('restoreHelpdeskSupportTicketStatus') && $showTrash,
		'tag' => 'button',
		'icon' => 'fa fa-undo',
		'options' => [
			...
			'data' => [
				...
				'dt-table' => '#dt-support-ticket-statuses',
			],
		],
	],
	[
		'visible' => Yii::$app->user->can('deleteHelpdeskSupportTicketStatus'),
		'tag' => 'button',
		'icon' => 'fa fa-trash',
		'options' => [
...
<?= DataTable::widget([
	...
	'clientOptions' => [
		...
		'serverSide' => true,
		'ajax' => [
			'url' => Url::to(['dt-support-ticket-statuses']),
			'method' => 'POST',
			'data' => new JsExpression('function (data) {
				data.deleted = ' . json_encode($showTrash ? SupportTicketStatus::YES : null) . ';
			}'),
		],
		'order' => [
			[2, 'asc'],
		...
		'columns' => [
			...
			[
				...
				'className' => 'col-autowidth',
				'filter' => ['select', ArrayHelper::getColumn(SupportTicketStatus::getStatusLabels(), 'label')],
			],
		],
		'rowCallback' => new JsExpression('function (row, data, index) {
			if (data.default && data.default.value == "' . SupportTicketStatus::YES . '") {
