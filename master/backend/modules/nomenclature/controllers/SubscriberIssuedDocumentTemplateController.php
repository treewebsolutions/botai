...
use backend\modules\nomenclature\models\SubscriberIssuedDocumentTemplateForm;
use backend\modules\nomenclature\models\SubscriberIssuedDocumentTemplateSearch;
...
class SubscriberIssuedDocumentTemplateController extends MainController
{
	/**
	 * @inheritdoc
	 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-subscriber-issued-document-templates'],
						'roles' => ['manageSubscriberIssuedDocumentTemplate'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
	...
	{
		return [
			'dt-subscriber-issued-document-templates' => SubscriberIssuedDocumentTemplateSearch::class,
		];
	}
	...
	{
		$model = new SubscriberIssuedDocumentTemplateForm();

		Yii::$app->eventLog->beginRecord($model);

		if ($model->load(Yii::$app->request->post()) && $model->saveModel()) {
			TagDependency::invalidate(Yii::$app->cache, Template::SUBSCRIBER_ISSUED_DOCUMENT . 'findAllTemplates');
	...
	{
		$model = $this->findModel($id, SubscriberIssuedDocumentTemplateForm::class);
	 ...
	 * @return \yii\db\ActiveRecord|SubscriberIssuedDocumentTemplateForm the loaded model
	 * @throws NotFoundHttpException if the model cannot be found
	 */
	protected function findModel($id, $modelName = null)
	{
		...
		$model = $modelName::findOne([
			'id' => $id,
			'type' => Template::SUBSCRIBER_ISSUED_DOCUMENT,
		]);
