		...
		<div class="form-fields <?= Yii::$app->request->isAjax ? 'modal-body' : '' ?>">
			<div class="row">
				...
				<div class="col-sm-3">
					...
					])->hint(Yii::t('backend', 'An active OpenAI integration is offered to every workspace that has not configured one of its own.')) ?>
				</div>
				<div class="col-sm-3">
			...
			<div class="row">
				<div class="col-sm-12">
					...
					<?= $form->field($model, 'apiKey', ['template' => $secretFieldTemplate])->passwordInput([
						// Never rendered back: a stored key stays out of the markup, and a
						// blank submission leaves it as it was.
						'value' => '',
						'placeholder' => $model->getIsNewRecord() ? '' : Yii::t('backend', 'Unchanged. Type a new value to replace it.'),
					]) ?>
				</div>
			</div>
		</div>
