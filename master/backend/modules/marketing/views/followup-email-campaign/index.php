...
if ($showTrash = Yii::$app->settings->get('enableSoftDelete')) {
	$showTrash = Yii::$app->request->get('deleted') == FollowupMarketingCampaign::YES;
	$this->params['breadcrumbs'][] = [
		'template' => '<li class="page-links">{link}</li>',
		'label' => implode('', [
			Html::a(Yii::t('common', 'Current'), ['index'], ['class' => $showTrash ? '' : 'active']),
			Html::a(Yii::t('common', 'Trash'), ['index', 'deleted' => FollowupMarketingCampaign::YES], ['class' => $showTrash ? 'active' : '']),
		]),
	];
}
...
$this->params['actions'] = [
	[
		'visible' => Yii::$app->user->can('restoreFollowupMarketingCampaign') && $showTrash,
		'tag' => 'button',
		'icon' => 'fa fa-undo',
		'options' => [
			...
			'data' => [
				...
				'dt-table' => '#dt-followup-email-campaigns',
			],
		],
	],
	[
		'visible' => Yii::$app->user->can('deleteFollowupMarketingCampaign'),
		'tag' => 'button',
		'icon' => 'fa fa-trash',
		'options' => [
...
<?= DataTable::widget([
	'id' => 'dt-followup-email-campaigns',
	'options' => [
	...
	'clientOptions' => [
		...
		'serverSide' => true,
		'ajax' => [
			'url' => Url::to(['dt-followup-email-campaigns']),
			'method' => 'POST',
			'data' => new JsExpression('function (data) {
				data.deleted = ' . json_encode($showTrash ? FollowupMarketingCampaign::YES : null) . ';
			}'),
		...
		'columns' => [
			...
			[
				'data' => 'frequency',
				'title' => Yii::t('label', 'Start Interval'),
				'orderable' => false,
				'searchable' => false,
				'filter' => false,
			],
			[
				'data' => 'event',
				'title' => Yii::t('label', 'Event'),
				'filter' => ['select', FollowupMarketingCampaign::getEventLabels()],
			],
			[
				'data' => 'start_at',
			...
			[
				...
				'filter' => ['select', ArrayHelper::getColumn(FollowupMarketingCampaign::getStatusLabels(), 'label')],
			],
		],
	],
]) ?>
