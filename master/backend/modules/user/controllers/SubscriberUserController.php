...
use backend\modules\user\models\SubscriberUserForm;
use backend\modules\user\models\SubscriberUserSearch;
...
class SubscriberUserController extends MainController
{
	/**
	 * @inheritdoc
	 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-subscriber-users', 'find-existing-users'],
						'roles' => ['viewUser'],
					],
					[
						'allow' => true,
	...
	{
		return [
			'dt-subscriber-users' => SubscriberUserSearch::class,
		];
	}
	...
	{
		$model = $this->findModel($id, SubscriberUserForm::class);
		$result = true;
    ...
    public function actionDelete($id = null)
    {
        ...
        try {
                ...
                Yii::$app->eventLog
                    ...
                    if ($model->delete($isPermanent)) {
                        ...
                        foreach ($workspaces as $workspace) {
                            try {
                                if ($workspace->uninstall()) {
                                    Workspace::deleteAll(['id' => $workspace->id]);
                                    continue;
                                } else {
                                    throw new \Exception('Cannot uninstall workspace');
                                }
                            } catch (\Exception $e) {
                                return false;
                            }
            ...
            Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllProducts']));
	 ...
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|User|SubscriberUserForm
	 * @throws NotFoundHttpException if no model was found
	 */
