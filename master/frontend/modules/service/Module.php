...
class Module extends \yii\base\Module implements \yii\base\BootstrapInterface
{
	...
	protected function buildUrlRules()
	{
		...
		foreach ($pages as $page) {
			...
			if ($page->controller == 'default') {
				$urlRules = array_merge($urlRules, $this->buildServiceUrlRules($page, $pageTranslation));
				continue;
			}
	...
	/**
	 * Builds URL Rules for ServiceController.
	 *
	 * @param \common\models\Page $page
	 * @param \common\models\PageTranslation $pageTranslation
	 * @return array
	 */
	protected function buildServiceUrlRules($page, $pageTranslation)
	{
		$route = "{$page->controller}/{$page->action}";

		$urlRules[] = [
			'pattern' => Inflector::slug(Yii::t($this->id, 'Module')) . "/<category>",
			'route' => $route,
		];
		$urlRules[] = [
			'pattern' => "<slug>",
