...
class StringHelper extends \yii\helpers\BaseStringHelper
{
	...
	public static function highlightText($text, $terms){
		// Decode HTML entities in the text
		$text = html_entity_decode($text, ENT_QUOTES | ENT_HTML401, 'UTF-8');

		// Sort terms based on whether they are substrings of other terms
		usort($terms, function($a, $b) use ($terms) {
			return static::compareTerms($a, $b, $terms);
		});

		// Normalize the text
		$textNormalized = Inflector::transliterate($text);

		// Create patterns for each term
		$patterns = array_map(function($term) {
			$termNormalized = Inflector::transliterate($term);
			return '\b(' . preg_quote($term, '/') . '|' . preg_quote($termNormalized, '/') . ')\b';
		}, $terms);

		// Create the final pattern
		$pattern = '/(' . implode('|', $patterns) . ')/i';

		// Replace the matched terms with highlighted versions
		$highlightedText = preg_replace_callback($pattern, function($matches) {
			$matchedTerm = $matches[0];
			return '<span class="label label-info">' . $matchedTerm . '</span>';
		}, $textNormalized);

		return $highlightedText;
	}

	private static function compareTerms($a, $b, $terms) {
		$isSubStringA = static::isSubstringOfAnotherTerm($a, $terms);
		$isSubStringB = static::isSubstringOfAnotherTerm($b, $terms);

		if ($isSubStringA && !$isSubStringB) {
			return 1;
		} elseif ($isSubStringB && !$isSubStringA) {
			return -1;
		} else {
			return 0;
		}
	...
	private static function isSubstringOfAnotherTerm($term, $terms) {
		foreach ($terms as $word) {
			if ($word !== $term && strpos($word, $term) !== false) {
				return true;
			}
		}
		return false;
