	...
	public function actionDelete($id = null)
	{
		...
		try {
			...
			foreach ($models->each() as $model) {
				...
				// Check if User has a Customer model associated
				if ($model->getCustomer()->exists()) {
					// Perform a delete operation just for user assigned roles and permissions
			...
			if ($isPermanent) {
				...
				// Since there is no relation between User and AuthAssignment at DB level,
				// manually delete all user assigned roles and permissions
				AuthAssignment::deleteAll(['user_id' => $deletedModels]);
			}
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllUser']));
