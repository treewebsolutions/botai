...
class PageFileForm extends File
{
	/**
	...
	/**
	 * @var UploadedFile The page file.
	 */
	public $fileFile;

	/**
	 * @var array The multilingual title of the page file.
	 */
	public $title = [];
	...
	public function rules()
	{
        ...
        return attribute.$form.find("[name*=\"[title][' . Yii::$app->language . ']\"]").val() == "";
			...
			[['fileFile'], 'file', 'maxSize' => Yii::$app->settings->get('maxFileSize'), 'maxFiles' => $this->isNewRecord ? 15 : 1, 'skipOnEmpty' => true],
	...
	public function attributeLabels()
	{
		return ArrayHelper::merge(parent::attributeLabels(), [
			...
			'fileFile' => $this->isNewRecord ? Yii::t('label', 'Files') : Yii::t('label', 'File'),
	...
	public function afterFind()
	{
		...
		$this->title = ArrayHelper::map($this->fileTranslations, 'language_id', 'title');
	}
	...
	{
		try {
			if (!($file = UploadedFile::getInstance($this, 'fileFile'))) {
				return true;
			}

			$dirPath = Yii::getAlias("@uploads/file/{$this->id}");
	...
	/**
	 * Saves the FileTranslation models.
	 *
	 * @return bool
	 */
	protected function saveFileTranslations()
	{
		try {
			...
			foreach (Language::findAllLanguages() as $language) {
				$fileTranslation = FileTranslation::findOne([
					'file_id' => $this->id,
					'language_id' => $language->language_id,
				]);
				if (!$fileTranslation) {
					$fileTranslation = new FileTranslation();
					$fileTranslation->file_id = $this->id;
					$fileTranslation->language_id = $language->language_id;
				}
				if (!empty($this->translator)) {
	...
	{
		try {
			if (!($files = UploadedFile::getInstances($this, 'fileFile'))) {
				return true;
			}
			$page = $this->getPage();
			...
			foreach ($files as $file) {
				$model = new File();
				$model->file = '_blank';
				$model->type = File::TYPE_PAGE;
				$model->status = static::STATUS_ACTIVE;
				if (!$model->save()) {
					throw new \Exception();
				}
				$pageHasFile = new PageHasFile();
				$pageHasFile->page_id = $page->id;
				$pageHasFile->file_id = $model->id;
				if (!$pageHasFile->save()) {
					throw new \Exception();
				}
				...
				foreach (Language::findAllLanguages() as $language) {
					$modelTranslation = new FileTranslation();
					$modelTranslation->file_id = $model->id;
				...
				$dirPath = Yii::getAlias("@uploads/file/{$model->id}");
				$fileName = StringHelper::truncate(implode('_', array_filter([
				...
				if (!$model->updateAttributes(['file' => $fileName])) {
					throw new \Exception();
				}
			}
			return true;
