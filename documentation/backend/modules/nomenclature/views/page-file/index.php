...
if ($this->context->id != 'page') {
	...
	if ($showTrash = Yii::$app->settings->get('enableSoftDelete')) {
		$showTrash = Yii::$app->request->get('deleted') == File::YES;
		$this->params['breadcrumbs'][] = [
			'template' => '<li class="page-links">{link}</li>',
			'label' => implode('', [
				Html::a(Yii::t('common', 'Current'), ['index', 'page_id' => $page_id], ['class' => $showTrash ? '' : 'active']),
				Html::a(Yii::t('common', 'Trash'), ['index', 'deleted' => File::YES, 'page_id' => $page_id], ['class' => $showTrash ? 'active' : '']),
			]),
		];
	}
...
<?= DataTable::widget([
	...
	'clientOptions' => [
		...
		'serverSide' => true,
		'ajax' => [
			'url' => Url::to(['page-file/dt-page-files', 'page_id' => $page_id]),
			'method' => 'POST',
			'data' => new JsExpression('function (data) {
				data.page_id = ' . json_encode($page_id) . ';
				data.deleted = ' . json_encode($showTrash ? File::YES : null) . ';
			}'),
		],
		'order' => [
			[2, 'asc'],
		...
		'columns' => [
			...
			[
				...
				'className' => 'col-autowidth',
				'filter' => ['select', ArrayHelper::getColumn(File::getStatusLabels(), 'label')],
			],
		],
	],
]) ?>
...
<?php
$this->registerJs('
	$("#dt-page-files").on("draw.dt", function () {
        $("#dt-page-files").parent().doubleScroll();
		$(".doubleScroll-scroll").width($("#dt-page-files").outerWidth());
	});
	$("#dt-page-files").on("column-visibility.dt", function () {
        $("#dt-page-files").parent().doubleScroll();
		$(".doubleScroll-scroll").width($("#dt-page-files").outerWidth());
	});
