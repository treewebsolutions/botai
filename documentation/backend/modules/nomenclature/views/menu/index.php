...
if ($showTrash = Yii::$app->settings->get('enableSoftDelete')) {
	$showTrash = Yii::$app->request->get('deleted') == Menu::YES;
	$this->params['breadcrumbs'][] = [
		'template' => '<li class="page-links">{link}</li>',
		'label' => implode('', [
			Html::a(Yii::t('common', 'Current'), ['index'], ['class' => $showTrash ? '' : 'active']),
			Html::a(Yii::t('common', 'Trash'), ['index', 'deleted' => Menu::YES], ['class' => $showTrash ? 'active' : '']),
		]),
	];
}
	...
	'clientOptions' => [
		...
		'ajax' => [
			...
			'data' => new JsExpression('function (data) {
				data.deleted = ' . json_encode($showTrash ? Menu::YES : null) . ';
			}'),
		],
		'order' => [
			[3, 'asc'],
		...
		'columns' => [
			...
			[
				'data' => 'position',
				'title' => Yii::t('label', 'Position'),
				'filter' => ['select', Menu::getPositionLabels()],
			],
			[
				'data' => 'default',
				'title' => Yii::t('label', 'Default'),
				'filter' => ['select', Menu::getBooleanLabels()],
			],
			[
				'data' => 'created_by',
			...
			[
				...
				'filter' => ['select', ArrayHelper::getColumn(Menu::getStatusLabels(), 'label')],
			],
		],
		'rowCallback' => new JsExpression('function (row, data, index) {
			if (data.defaultValue == "' . Menu::YES . '") {
...
<?php
$this->registerJs('
	$("#dt-menus").on("draw.dt", function () {
        $("#dt-menus").parent().doubleScroll();
		$(".doubleScroll-scroll").width($("#dt-menus").outerWidth());
	});
	$("#dt-menus").on("column-visibility.dt", function () {
        $("#dt-menus").parent().doubleScroll();
		$(".doubleScroll-scroll").width($("#dt-menus").outerWidth());
	});
