...
use backend\modules\nomenclature\models\PageFileForm;
use backend\modules\nomenclature\models\PageFileSearch;
...
class PageFileController extends MainController
{
	/**
	 * @inheritdoc
	 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-page-files'],
						'roles' => ['viewPage'],
					],
					[
						'allow' => true,
	...
	{
		return [
			'dt-page-files' => PageFileSearch::class,
		];
	}
	...
	{
		$model = $this->findModel($id, PageFileForm::class);
		...
		try {
			...
			if ($isPermanent) {
				foreach ($deletedModels as $deletedModel) {
					FileHelper::removeDirectory(Yii::getAlias("@uploads/file/{$deletedModel}"));
				}
			}
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllPages']));
	 ...
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|PageFileForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : File::class;
		$query = $modelName::find()
			->alias('p')
			->joinWith([
				'pageHasFiles php' => function (ActiveQuery $query) {
					$query->andWhere(['php.page_id' => Yii::$app->request->get('page_id')]);
				},
			], false)
			->andWhere([
				'p.id' => $id,
				'p.type' => File::TYPE_PAGE,
			]);
		...
		$query->andWhere(['p.deleted' => File::NO]);
