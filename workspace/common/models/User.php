...
class User extends CommonActiveRecord implements \yii\web\IdentityInterface
{
	 ...
	 *
	 * The address is the only credential the platform accepts; `username` and `phone`
	 * are descriptive columns nothing authenticates against.
	 *
	 * @param string $email
	...
	/**
	 * Finds the accounts with no role assigned that carry the given email address.
	 *
	 * The user form's autofill calls this to pre-fill an existing person's details.
	 * It used to call findAllUsersWithoutRoleByUsername(), which this model never
	 * defined, so the lookup died with "call to undefined method" every time.
	 *
	 * @param string $email
