...
class Subscription extends CommonActiveRecord
{
	...
	public function rules()
	{
		return [
			...
			[['price'], 'number'],
			[['code', 'trial_cycle', 'billing_cycle'], 'string', 'max' => 255],
	...
	/**
	 * Gets the SubscriptionFeature models by module name.
	 *
	 * @param string $moduleName The module name.
	 * @return \yii\db\ActiveRecord[]|SubscriptionFeature[]|null
	 */
	public function getSubscriptionFeaturesByModuleName($moduleName)
	{
		$subscriptionQuery = Subscription::find()
			->select(['id'])
			->andWhere([
		...
		return SubscriptionFeature::find()
			...
			->joinWith([
				'feature.featureModule fm' => function (ActiveQuery $query) use ($moduleName) {
					$query->andWhere(['fm.name' => $moduleName]);
				},
			])
			->andWhere([
				'sf.subscription_id' => $subscriptionQuery,
	...
	public function getFormattedName($asHtml = false)
	{
        ...
        if ($this->package->translation->name) {
