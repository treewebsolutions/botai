...
class UploadHelper
{
	...
	public static function getUploadsUrl($scheme = true)
	{
		...
		return str_replace($search, '', Url::base($scheme)) . "/uploads";
	}
	 ...
	 */
	public static function saveFile($uploadedFile, $fileName = '', $path = '', $suffix = true, $copy = false, $slug = true)
	{
		...
		$fileName = $fileName ? ($slug ? Inflector::slug($fileName) : $fileName) : $uploadedFile->getBaseName();
		// Add file name custom or random generated suffix
		$fileNameSuffix = '';
		if (!empty($suffix)) {
			$fileNameSuffix = '-' . (is_bool($suffix) ? Yii::$app->security->generateRandomString(8) : $suffix);
	 ...
	 * @throws \yii\base\Exception
	 */
	public static function saveImage($uploadedFile, $fileName = '', $path = '', $width = null, $height = null, $crop = false, $transparency = true, $suffix = true, $copy = false, $slug = true)
	{
		$new_img = ''; $new_image = '';
