...
{
	/**
	 * @var array the <html> tag HTML default attributes.
	 */
	...
	public function init()
	{
		...
		if (!Yii::$app->user->isGuest) {
			...
			// Set the last activity for the currently authenticated user
			...
			if ($workspaceFeature = \common\models\master\WorkspaceHasSubscriptionFeature::find()
				->alias('whsf')
				->select([
					'*',
				])
				...
				->where([
					'whsf.workspace_id' => $workspace->id,
					'sf.subscription_id' => $workspace->subscription_id,
					'sf.name' => Feature::WORKSPACES,
					'sf.deleted' => Feature::NO,
				])
				->one()) {
				$workspaceFeature->quota = 1;
				$workspaceFeature->save(false, ['quota']);
			}

			$statistics = $workspace->getWorkspaceSubscriptionFeatureStatistics(Feature::USERS);
			$counter = count(\common\models\User::findAllUsers());
			if (!$statistics['quota'] != $counter) {
				if ($counter > 0) {
					if ($workspaceFeature = \common\models\master\WorkspaceHasSubscriptionFeature::find()
						->alias('whsf')
						->select([
						...
						->where([
							'whsf.workspace_id' => $workspace->id,
							'sf.subscription_id' => $workspace->subscription_id,
							'sf.name' => Feature::USERS,
							'sf.deleted' => Feature::NO,
						])
						->one()) {
						$workspaceFeature->quota = $counter;
						$workspaceFeature->save(false, ['quota']);
					}
				}
			}
