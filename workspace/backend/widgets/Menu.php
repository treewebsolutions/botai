...
class Menu extends \yii\widgets\Menu
{
	...
	/**
	 * Filters the items list by ignoring the keys given as argument.
	 *
	 * @param array $data
	 * @param null|array $ignore
	 * @return array
	 */
	public static function ignoreItems($data, $ignore)
	{
		if (empty($ignore)) {
			return $data;
		}
		...
		foreach ($data as $key => $val) {
			...
			if (isset($val['items'])) {
				// Filter the the child items
				$items = self::ignoreItems($val['items'], $ignore[$key]);
				// Push to the stack only if the child items array is not empty or has toggle set
				if (!empty($items) || isset($val['toggle'])) {
					$filteredItems[$key] = [
						'roles' => $val['roles'],
