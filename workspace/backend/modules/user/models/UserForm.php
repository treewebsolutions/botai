...
use common\models\master\Page;
use common\models\master\Template;
...
class UserForm extends User
{
	...
	public function init()
	{
		...
		if (Yii::$app->masterSettings->get('userAccountActivation') == static::ACCOUNT_ACTIVATION_CONFIRMATION) {
	...
	public function sendWelcomeEmail()
	{
		try {
			...
			$shortCodeValues = [
				...
				'{{APP_LOGO_URL}}' => Url::to('@uploads/' . Yii::$app->masterSettings->get('appLogo'), true) ?: Url::to('@frontend/web/img/logo.png', true),
				'{{APP_LOGO_ALT_URL}}' => Url::to('@uploads/' . Yii::$app->masterSettings->get('appLogoAlt'), true) ?: Url::to('@frontend/web/img/logo-alt.png', true),
	...
	public function sendActivationEmail()
	{
		...
		if (!($templateTranslation = $template->getTranslation()) || !($page = Page::findPageByRoute(['/site/activate'])->getTranslation())) {
			$templateTranslation = $template->getTranslation(Yii::$app->masterSettings->get('defaultLanguage'));
			$page = Page::findPageByRoute(['/site/activate'])->getTranslation(Yii::$app->masterSettings->get('defaultLanguage'));
			$language = mb_substr(Yii::$app->masterSettings->get('defaultLanguage'), 0, 2);
		...
		$shortCodeValues = [
			'{{APP_NAME}}' => Yii::$app->name,
			'{{APP_URL}}' => $appUrl,
			'{{APP_LOGO_URL}}' => Url::to('@uploads/' . Yii::$app->masterSettings->get('appLogo'), true) ?: Url::to('@web/img/logo.png', true),
			'{{APP_LOGO_ALT_URL}}' => Url::to('@uploads/' . Yii::$app->masterSettings->get('appLogoAlt'), true) ?: Url::to('@web/img/logo-alt.png', true),
			'{{FIRST_NAME}}' => $this->first_name,
			'{{MIDDLE_NAME}}' => $this->middle_name,
			'{{LAST_NAME}}' => $this->last_name,
	...
	public function save($runValidation = true, $attributeNames = null)
	{
		...
		try {
			...
			if ($isNewRecord = $this->getIsNewRecord()) {
				$user = \common\models\master\User::findByAttributes(['email' => $this->email]);
				if (!empty($user)) {
			...
			if ($isNewRecord && Yii::$app->masterSettings->get('userAccountActivation') == static::ACCOUNT_ACTIVATION_CONFIRMATION) {
