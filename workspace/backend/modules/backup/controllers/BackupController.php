...
class BackupController extends MainController
{
	...
	public function actionCreate()
	{
		...
		try {
			...
			], Yii::getAlias('@backups/' . $workspace['id'] . '-' .$workspace['code'] . '/' . $workspace['id'] . '-' . $workspace['code'] . '-' . date('Y.m.d-h.i.s', strtotime($model->created_at)) . '.zip'));
	...
	public function actionDownload($id)
	{
		...
		return Yii::$app->response->sendFile(Yii::getAlias('@backups/' . $workspace['id'] . '-' .$workspace['code'] . '/' . $workspace['id'] . '-' . $workspace['code'] . '-' . date('Y.m.d-h.i.s', strtotime($model->created_at)) . '.zip'));
	}
	...
	public function actionDelete($id = null)
	{
		...
		try {
			...
			foreach ($models->each() as $model) {
                ...
                $code = end(explode('_', Yii::$app->db->dsn));
                $workspace = Workspace::findOne(['code' => $code]);
                $file = Yii::getAlias('@backups/' . $workspace['id'] . '-' .$workspace['code'] . '/' . $workspace['id'] . '-' . $workspace['code'] . '-' . date('Y.m.d-h.i.s', strtotime($model->created_at)) . '.zip');
                if ($isPermanent && is_file($file)) {
                    unlink($file);
                }
			}
