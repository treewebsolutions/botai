...
$this->title = Yii::t('common', 'View {item}', ['item' => Yii::t('common', 'Conversation')]);
...
$this->params['actions'] = [
	[
		'visible' => Yii::$app->user->can('viewConversation'),
		'tag' => 'a',
		'url' => ['index'],
		'icon' => 'fa fa-list',
		'options' => [
	...
	[
		'visible' => Yii::$app->user->can('updateConversation'),
		'tag' => 'a',
	...
	[
		'visible' => Yii::$app->user->can('deleteConversation'),
		'tag' => 'a',
	...
	[
		'visible' => Yii::$app->user->can('createConversation'),
		'tag' => 'a',
		'url' => ['create'],
		'icon' => 'fa fa-plus',
		'options' => [
			...
			<div class="table-responsive">
				<?= DetailView::widget([
					...
					'attributes' => [
						[
							'format' => 'html',
							'label' => Yii::t('label', 'Summary'),
							'value' => function (Conversation $model) {
								return $model->summary ? Html::encode($model->summary) : '&mdash;';
							},
						],
						[
							'format' => 'html',
							'label' => Yii::t('label', 'OpenAI Conversation ID'),
							'value' => function (Conversation $model) {
...
<?php if (!Yii::$app->request->isAjax && Yii::$app->user->can('viewMessage')): ?>
	<div class="portlet box blue-hoki mt-15">
		<div class="portlet-title">
			<div class="caption"><?= Yii::t('label', 'Messages') ?></div>
		</div>
		<div class="portlet-body">
			<?php $messages = $model->getMessages()->andWhere(['deleted' => \common\models\Message::NO])->orderBy(['id' => SORT_ASC])->all(); ?>
			...
			<?php else: ?>
				<table class="table table-striped table-bordered">
					<thead>
						<tr>
							<th class="col-autowidth"><?= Yii::t('label', 'Role') ?></th>
							<th><?= Yii::t('label', 'Content') ?></th>
							<th class="col-autowidth"><?= Yii::t('label', 'Status') ?></th>
							<th class="col-autowidth"><?= Yii::t('label', 'Created At') ?></th>
						</tr>
					</thead>
					<tbody>
						<?php foreach ($messages as $message): ?>
							<?php $status = \common\models\Message::getStatusLabels()[$message->status] ?? null; ?>
							<tr>
								<td><?= \common\models\Message::getRoleLabels()[$message->role] ?? Html::encode($message->role) ?></td>
								<td><?= nl2br(Html::encode($message->content)) ?></td>
								<td><?= $status ? Html::tag('span', $status['label'], ['class' => 'label label-' . $status['color']]) : '&mdash;' ?></td>
								<td><?= $message->created_at ? Yii::$app->formatter->asDatetime($message->created_at) : '&mdash;' ?></td>
							</tr>
						<?php endforeach; ?>
					</tbody>
				</table>
				...
				'clientOptions' => [
					...
					'ajax' => [
						...
						'data' => new JsExpression('function (data) {
							data.model = ' . json_encode(\common\models\Conversation::class) . '; 
