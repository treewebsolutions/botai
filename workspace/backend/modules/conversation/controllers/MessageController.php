...
use backend\modules\conversation\models\MessageForm;
...
class MessageController extends MainController
{
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-messages'],
						'roles' => ['viewMessage'],
					],
					[
						'allow' => true,
	...
	{
		return [
			'dt-messages' => MessageSearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == Message::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreMessage')) {
	...
	{
		$model = $this->findModel($id, MessageForm::class);
		$result = true;
	...
	/**
	 * Deletes existing Message models.
	...
	/**
	 * Restores Message models that are marked as deleted.
	...
	/**
	 * Finds the Message model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Message|MessageForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		...
		$query->andWhere(['deleted' => Message::NO]);
