...
use backend\modules\conversation\models\ConversationForm;
use backend\modules\conversation\models\ConversationSearch;
...
class ConversationController extends MainController
{
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-conversations'],
						'roles' => ['viewConversation'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
						'roles' => ['createConversation'],
					],
					[
						'allow' => true,
						'actions' => ['update'],
						'roles' => ['updateConversation'],
					],
					[
						'allow' => true,
						'actions' => ['delete'],
						'roles' => ['deleteConversation'],
						'verbs' => ['POST'],
					],
					[
						'allow' => true,
						'actions' => ['restore'],
						'roles' => ['restoreConversation'],
						'verbs' => ['POST'],
					],
				],
			],
	...
	{
		return [
			'dt-conversations' => ConversationSearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == Conversation::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreConversation')) {
	...
	{
		$model = new ConversationForm();
		$result = true;

		Yii::$app->eventLog->beginRecord($model);
		if ($model->load(Yii::$app->request->post()) && ($result = $model->save())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllConversations']));
	...
	{
		$model = $this->findModel($id, ConversationForm::class);
		$result = true;
	...
	/**
	 * Deletes existing Conversation models.
	...
	/**
	 * Restores Conversation models that are marked as deleted.
	...
	/**
	 * Finds the Conversation model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Conversation|ConversationForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Conversation::class;
		...
		$query->andWhere(['deleted' => Conversation::NO]);
