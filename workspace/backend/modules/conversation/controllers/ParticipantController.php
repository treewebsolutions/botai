...
namespace backend\modules\conversation\controllers;

use backend\controllers\MainController;
use backend\modules\conversation\models\ParticipantForm;
use backend\modules\conversation\models\ParticipantSearch;
...
class ParticipantController extends MainController
{
	/**
	 * @inheritdoc
	 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-participants'],
						'roles' => ['viewParticipant'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
						'roles' => ['createParticipant'],
					],
					[
						'allow' => true,
						'actions' => ['update'],
						'roles' => ['updateParticipant'],
					],
					[
						'allow' => true,
						'actions' => ['delete'],
						'roles' => ['deleteParticipant'],
						'verbs' => ['POST'],
					],
					[
						'allow' => true,
						'actions' => ['restore'],
						'roles' => ['restoreParticipant'],
						'verbs' => ['POST'],
					],
				],
			],
	...
	{
		return [
			'dt-participants' => ParticipantSearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == Participant::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreParticipant')) {
	...
	public function actionCreate()
	{
		...
		if ($model->load(Yii::$app->request->post()) && ($result = $model->save())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllParticipants']));
	...
	{
		$model = $this->findModel($id, ParticipantForm::class);
		$result = true;
	...
	/**
	 * Deletes existing Participant models.
	...
	/**
	 * Restores Participant models that are marked as deleted.
	...
	/**
	 * Finds the Participant model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Participant|ParticipantForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Participant::class;
		...
		$query->andWhere(['deleted' => Participant::NO]);
