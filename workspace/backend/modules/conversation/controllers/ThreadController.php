...
use backend\modules\conversation\models\ThreadForm;
use backend\modules\conversation\models\ThreadSearch;
...
class ThreadController extends MainController
{
	...
	{
		if (Yii::$app->request->get('deleted') == Thread::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreThread')) {
	...
	public function actionCreate()
	{
		...
		if ($model->load(Yii::$app->request->post()) && ($result = $model->save())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllThreads']));
	...
	{
		$model = $this->findModel($id, ThreadForm::class);
		$result = true;
	...
	/**
	 * Deletes existing Thread models.
	...
	/**
	 * Restores Thread models that are marked as deleted.
	...
	/**
	 * Finds the Thread model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Thread|ThreadForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Thread::class;
		...
		$query->andWhere(['deleted' => Thread::NO]);
