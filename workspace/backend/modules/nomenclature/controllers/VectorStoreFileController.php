...
use backend\modules\nomenclature\models\VectorStoreFileForm;
use backend\modules\nomenclature\models\VectorStoreFileSearch;
...
class VectorStoreFileController extends MainController
{
	/**
	 * @inheritdoc
	 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-vector-store-files'],
						'roles' => ['viewVectorStoreFile'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
						'roles' => ['createVectorStoreFile'],
					],
					[
						'allow' => true,
						'actions' => ['update'],
						'roles' => ['updateVectorStoreFile'],
					],
					[
						'allow' => true,
						'actions' => ['delete'],
						'roles' => ['deleteVectorStoreFile'],
						'verbs' => ['POST'],
					],
					[
						'allow' => true,
						'actions' => ['restore'],
						'roles' => ['restoreVectorStoreFile'],
						'verbs' => ['POST'],
					],
				],
			],
	...
	{
		return [
			'dt-vector-store-files' => VectorStoreFileSearch::class,
		];
	}

	/**
	 * Lists all VectorStoreFile models.
	 *
	 * @return mixed
	 */
	public function actionIndex()
	{
		if (Yii::$app->request->get('deleted') == VectorStoreFile::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreVectorStoreFile')) {
	...
	/**
	 * Displays a single VectorStoreFile model.
	 *
	 * @param integer $id
	 * @return mixed
	...
	/**
	 * Creates a new VectorStoreFile model.
	...
	{
		$model = new VectorStoreFileForm();
		$result = true;

		Yii::$app->eventLog->beginRecord($model);
		if ($model->load(Yii::$app->request->post()) && ($result = $model->save())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllVectorStoreFiles']));
	...
	/**
	 * Updates an existing VectorStoreFile model.
	...
	{
		$model = $this->findModel($id, VectorStoreFileForm::class);
		$result = true;
	...
	/**
	 * Deletes existing VectorStoreFile models.
	...
	public function actionDelete($id = null)
	{
		...
		try {
			$deletedModels = [];
			/** @var VectorStoreFile $model */
			foreach ($models->each() as $model) {
				if ($model->default == VectorStoreFile::YES) {
	...
	/**
	 * Restores VectorStoreFile models that are marked as deleted.
	...
	/**
	 * Finds the VectorStoreFile model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|VectorStoreFile|VectorStoreFileForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : VectorStoreFile::class;
		...
		$query->andWhere(['deleted' => VectorStoreFile::NO]);
