...
use backend\modules\nomenclature\models\IntegrationForm;
use backend\modules\nomenclature\models\IntegrationSearch;
...
class IntegrationController extends MainController
{
	...
	{
		if (Yii::$app->request->get('deleted') == Integration::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreIntegration')) {
	...
	/**
	 * Deletes existing Integration models.
	...
	public function actionDelete($id = null)
	{
		...
		try {
			...
			foreach ($models->each() as $model) {
				if ($model->default == Integration::YES) {
	...
	/**
	 * Restores Integration models that are marked as deleted.
	...
	/**
	 * Finds the Integration model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Integration|IntegrationForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Integration::class;
		...
		$query->andWhere(['deleted' => Integration::NO]);
