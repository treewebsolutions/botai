...
use backend\modules\nomenclature\models\VectorStoreForm;
use backend\modules\nomenclature\models\VectorStoreSearch;
...
class VectorStoreController extends MainController
{
	/**
	 * @inheritdoc
	 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-vector-stores'],
						'roles' => ['viewVectorStore'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
						'roles' => ['createVectorStore'],
					],
					[
						'allow' => true,
						'actions' => ['update'],
						'roles' => ['updateVectorStore'],
					],
					[
						'allow' => true,
						'actions' => ['delete'],
						'roles' => ['deleteVectorStore'],
						'verbs' => ['POST'],
					],
					[
						'allow' => true,
						'actions' => ['restore'],
						'roles' => ['restoreVectorStore'],
						'verbs' => ['POST'],
					],
				],
			],
	...
	{
		return [
			'dt-vector-stores' => VectorStoreSearch::class,
		];
	}

	/**
	 * Lists all VectorStore models.
	 *
	 * @return mixed
	 */
	public function actionIndex()
	{
		if (Yii::$app->request->get('deleted') == VectorStore::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreVectorStore')) {
	...
	/**
	 * Displays a single VectorStore model.
	 *
	 * @param integer $id
	 * @return mixed
	...
	/**
	 * Creates a new VectorStore model.
	...
	{
		$model = new VectorStoreForm();
		$result = true;

		Yii::$app->eventLog->beginRecord($model);
		if ($model->load(Yii::$app->request->post()) && ($result = $model->save())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllVectorStores']));
	...
	/**
	 * Updates an existing VectorStore model.
	...
	{
		$model = $this->findModel($id, VectorStoreForm::class);
		$result = true;
	...
	/**
	 * Deletes existing VectorStore models.
	...
	/**
	 * Restores VectorStore models that are marked as deleted.
	...
	/**
	 * Finds the VectorStore model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|VectorStore|VectorStoreForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : VectorStore::class;
		...
		$query->andWhere(['deleted' => VectorStore::NO]);
